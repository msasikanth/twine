...
@Composable
internal fun ShowPinnedSourcesSettingItem(
  showPinnedSources: Boolean,
  onValueChanged: (Boolean) -> Unit,
) {
  var checked by remember(showPinnedSources) { mutableStateOf(showPinnedSources) }
  Box(
    modifier =
      Modifier.clickable {
        checked = !checked
        onValueChanged(!showPinnedSources)
      }
  ) {
    Row(
      ...
      Column(modifier = Modifier.weight(1f)) {
        Text(
          stringResource(Res.string.settingsShowPinnedSourcesTitle),
          style = MaterialTheme.typography.titleMedium,
          color = AppTheme.colorScheme.onSurface,
        )
        Text(
          stringResource(Res.string.settingsShowPinnedSourcesSubtitle),
