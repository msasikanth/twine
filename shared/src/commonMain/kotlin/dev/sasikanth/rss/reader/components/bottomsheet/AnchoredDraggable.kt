...
internal class AnchoredDraggableState<T>(
  ...
  internal val draggableState =
    object : DraggableState {
      ...
      override suspend fun drag(dragPriority: MutatePriority, block: suspend DragScope.() -> Unit) {
        this@AnchoredDraggableState.anchoredDrag(dragPriority) { with(dragScope) { block() } }
      }
   ...
   * successfully, but does not get reset when an animation gets interrupted. You can use this value
   * to provide smooth reconciliation behavior when re-targeting an animation.
   */
   ...
   * match the initial value's anchor. If the [newAnchors] are different to the existing anchors, or
   * there is no anchor for the [currentValue], the [onAnchorsChanged] callback will be invoked.
   *
   * <b>If your anchors depend on the size of the layout, updateAnchors should be called in the
   * layout (placement) phase, e.g. through Modifier.onSizeChanged.</b> This ensures that the state
   * is set up within the same frame. For static anchors, or anchors with different data
   * dependencies, updateAnchors is safe to be called any time, for example from a side effect.
   *
   * @param newAnchors The new anchors
   * @param onAnchorsChanged Optional callback to be invoked if the state needs to be updated after
   *   updating the anchors, for example if the anchor for the [currentValue] has been removed
   */
  ...
  suspend fun settle(velocity: Float) {
    ...
    val targetValue =
      computeTarget(offset = requireOffset(), currentValue = previousValue, velocity = velocity)
   ...
   *
   * All actions that change the [offset] of this [AnchoredDraggableState] must be performed within
   * an [anchoredDrag] block (even if they don't call any other methods on this object) in order to
   * guarantee that mutual exclusion is enforced.
   *
   ...
   * to arrive to. This will set [AnchoredDraggableState.targetValue] to provided value so consumers
   * can reflect it in their UIs.
   *
   * If [anchoredDrag] is called from elsewhere with the [dragPriority] higher or equal to ongoing
   * drag, ongoing drag will be canceled.
   *
   ...
   */
  internal fun trySnapTo(targetValue: T): Boolean =
    dragMutex.tryMutate {
  ...
  fun interface AnchorChangedCallback<T> {
    ...
    /**
     * Callback that is invoked when the anchors have changed, after the [AnchoredDraggableState]
     * has been updated with them. Use this hook to re-launch animations or interrupt them if
     * needed.
     *
...
internal object AnchoredDraggableDefaults {
  ...
  /**
   * A [AnchorChangedCallback] implementation that attempts to reconcile an in-progress animation by
    ...
    AnchorChangedCallback<T> { previousTarget, previousAnchors, newAnchors ->
      ...
      if (previousTargetOffset != newTargetOffset) {
        if (newTargetOffset != null) {
          scope.launch { state.animateTo(previousTarget, state.lastVelocity) }
        } else {
          scope.launch {
            state.snapTo(
              newAnchors.closestAnchor(offset = state.requireOffset(), searchUpwards = false)
            )
          }
        }
      }
...
private fun <T> Map<T, Float>.closestAnchor(offset: Float, searchUpwards: Boolean): T {
