...
internal class InternalMutatorMutex {
   ...
   * [priority] values are compared. If the new caller has a [priority] equal to or higher than the
   * call in progress, the call in progress will be cancelled, throwing [CancellationException] and
   * the new caller's [block] will be invoked. If the call in progress had a higher [priority] than
   * the new caller, the new caller will throw [CancellationException] without invoking [block].
   *
   ...
   * Enforce that only a single caller may be active at a time.
   *
   * If [mutateWith] is called while another call to [mutate] or [mutateWith] is in progress, their
   * [priority] values are compared. If the new caller has a [priority] equal to or higher than the
   * call in progress, the call in progress will be cancelled, throwing [CancellationException] and
   * the new caller's [block] will be invoked. If the call in progress had a higher [priority] than
   * the new caller, the new caller will throw [CancellationException] without invoking [block].
   ...
   * mutable scope to the provided [block] while leaving the rest of the state object read-only. For
   * example:
   *
  ...
  /**
   * Attempt to mutate synchronously if there is no other active caller. If there is no other active
   * caller, the [block] will be executed in a lock. If there is another active caller, this method
   * will return false, indicating that the active caller needs to be cancelled through a [mutate]
   * or [mutateWith] call with an equal or higher mutation priority.
   *
